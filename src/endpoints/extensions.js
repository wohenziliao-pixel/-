...
/**
 * EU 演示页 / 稳定页调用的无状态回传：把 payload 原样返回，供后续若需服务端加工时扩展。
 */
router.post('/eu-runtime-hook', (request, response) => {
    try {
        const stage = String(request.body?.stage || '').trim();
        const payload = request.body?.payload || {};

        if (stage === 'deleteConversationFiles') {
            const chatsRoot = request.user.directories.chats;
            const deletedFiles = [];
            const deletedFolders = [];
            const failed = [];
            const conversations = Array.isArray(payload?.conversations) ? payload.conversations : [];
            const storybookTitle = String(payload?.storybookTitle || '').trim();
            /** 仅开发者硬删资源等场景需要；用户「移除会话」默认必须为 false，避免整夹删掉酒馆侧角色目录。 */
            const nukeFolders = payload?.deleteEntireCharacterFolders === true;

            const tryDeleteChatFile = (rawFolder, rawChatFile) => {
                const folderName = sanitize(String(rawFolder || '').trim());
                const chatFile = sanitize(String(rawChatFile || '').trim());
                if (!folderName || !chatFile) return;
                const base = chatFile.endsWith('.jsonl') ? chatFile : `${chatFile}.jsonl`;
                const filePath = path.join(chatsRoot, folderName, base);
                try {
                    if (fs.existsSync(filePath) && fs.statSync(filePath).isFile()) {
                        fs.rmSync(filePath, { force: true });
                        deletedFiles.push(`${folderName}/${base}`);
                    }
                } catch (error) {
                    failed.push({ type: 'file', target: `${folderName}/${base}`, error: String(error?.message || error) });
                }
            };

            // 默认：只删 payload 列出的聊天文件，不递归删目录（否则会连坐同角色/书名下的其它聊天与酒馆数据）。
            for (const conv of conversations) {
                tryDeleteChatFile(conv?.characterName, conv?.chatFile);
            }

            if (nukeFolders) {
                const folderCandidates = new Set();
                conversations.forEach((conv) => {
                    const n = String(conv?.characterName || '').trim();
                    if (n) folderCandidates.add(sanitize(n));
                });
                if (storybookTitle) folderCandidates.add(sanitize(storybookTitle));

                for (const folderName of folderCandidates) {
                    if (!folderName) continue;
                    const folderPath = path.join(chatsRoot, folderName);
                    try {
                        ...
                        failed.push({ type: 'folder', target: folderName, error: String(error?.message || error) });
                    }
                }
            }
        ...
        return response.json({ ok: true, payload: request.body?.payload });
    }
});
