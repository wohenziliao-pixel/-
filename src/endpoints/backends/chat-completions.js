...
async function sendXaiRequest(request, response) {
    ...
    if (!apiKey && !request.body.reverse_proxy) {
        ...
        return response.status(400).send({ error: true, message: 'xAI API key is missing.' });
    }
        ...
        if (!response.headersSent) {
            response.status(502).send({
                ...
                message: error.code === 'ECONNREFUSED'
                    ...
                    : (error.message || 'Error communicating with xAI API'),
            });
        } else {
            response.end();
        }
...
/**
 * Normalize OpenAI-style messages so system prompt stays at top.
 * Some reverse proxies reject system messages in the middle.
 * @param {any[]} messages
 * @returns {any[]}
 */
function normalizeSystemMessagesAtFront(messages) {
    ...
    if (!systems.length) return others;
    return [{ role: 'system', content: systems.join('\n\n') }, ...others];
}

router.post('/generate', async function (request, response) {
    try {
        if (!request.body) return response.status(400).send({ error: true, message: 'Empty request body' });
        ...
        if (
            ...
            request.body.messages = normalizeSystemMessagesAtFront(request.body.messages);
        }
        ...
        if (request.body.chat_completion_source === CHAT_COMPLETION_SOURCES.OPENAI) {
            ...
            const src = String(request.body.chat_completion_source || '');
            console.warn('This chat completion source is not supported yet.', src);
            return response.status(400).send({
                error: true,
                message: `Unsupported chat completion source: ${src || '(empty)'}`,
            });
        }
        ...
        if (!apiKey && !request.body.reverse_proxy && request.body.chat_completion_source !== CHAT_COMPLETION_SOURCES.CUSTOM) {
            ...
            return response.status(400).send({ error: true, message: 'OpenAI API key is missing.' });
        }
